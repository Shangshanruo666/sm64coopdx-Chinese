...
function round_end()
    ...
    if gGlobalSyncTable.gameMode == GAME_MODE_DM or gGlobalSyncTable.gameMode == GAME_MODE_FT or gGlobalSyncTable.gameMode == GAME_MODE_KOTH then
        ...
        if winner ~= nil then
            ...
            gGlobalSyncTable.message = strip_colors(winnerNp.name) .. ' 获胜!'
        else
            gGlobalSyncTable.message = '回合结束'
        end
    elseif gGlobalSyncTable.gameMode == GAME_MODE_TDM or gGlobalSyncTable.gameMode == GAME_MODE_CTF or gGlobalSyncTable.gameMode == GAME_MODE_TFT or gGlobalSyncTable.gameMode == GAME_MODE_TKOTH then
        ...
        if redScore > blueScore then
            gGlobalSyncTable.message = '红队获胜!'
        elseif blueScore > redScore then
            gGlobalSyncTable.message = '蓝队获胜!'
        else
            gGlobalSyncTable.message = '回合结束'
        end
    else
...
function on_arena_player_death(victimGlobalId, attackerGlobalId)
    ...
    if npAttacker == nil or npVictim == npAttacker then
        ...
        djui_popup_create(victimColor .. npVictim.name .. normalColor .. " 已死亡!", 2)
        ...
        djui_popup_create(attackerColor .. npAttacker.name .. normalColor .. " 击杀了 " .. victimColor .. npVictim.name .. normalColor .. "!", 2)
...
function on_gamemode_command(msg)
    ...
    if msg == 'random' then
        djui_chat_message_create("[Arena] 设置为随机游戏模式")
        sRandomizeMode = true
        round_end()
        sWaitTimer = 1
        sRoundCount = 0
    ...
    if setMode ~= nil then
        djui_chat_message_create("[Arena] 设置游戏模式")
    ...
    djui_chat_message_create("/arena \\#00ffff\\gamemode\\#ffff00\\ " .. string.format("[%s|random]\\#dcdcdc\\ 设置游戏模式", sGameModeShortTimes))
    return true
end
...
function on_level_command(msg)
    ...
    djui_chat_message_create("/arena \\#00ffff\\level\\#ffff00\\ " .. string.format("[%s]\\#dcdcdc\\ 设置关卡", get_level_choices()))
    return true
end
...
function on_jump_leniency_command(msg)
    ...
    if not network_is_server and not network_is_moderator() then
        djui_chat_message_create("\\#ffa0a0\\[Arena] 您没有权限使用此命令")
        return true
    elseif num == nil then
        djui_chat_message_create("\\#ffa0a0\\[Arena] 无效数字!")
        return true
    else
        gGlobalSyncTable.jumpLeniency = num
        djui_chat_message_create("[Arena] 跳跃宽容帧的数量已设置为 " .. num)
        return true
    end
end
