...
function bhv_arena_flag_return(obj, showMessage)
    ...
    if showMessage then
        local msg = string.format('%s  %s%s%s 旗帜被重置!', team_color_str(otherTeam), team_color_str(obj.oArenaFlagTeam), team_name_str(obj.oArenaFlagTeam), team_color_str(otherTeam))
...
function bhv_arena_flag_collect(obj, m)
    ...
    if obj.oArenaFlagTeam > 0 and s.team == obj.oArenaFlagTeam then
        if obj.oArenaFlagAtBase == 1 then
            ...
            if otherFlag.oArenaFlagHeldByGlobal == np.globalIndex then
                ...
                local msg = string.format('%s%s 带回了 %s%s%s 旗帜!', team_color_str(s.team), strip_colors(np.name), team_color_str(otherTeam), team_name_str(otherTeam), team_color_str(s.team))
        ...
        obj.oPosZ = data.pos.z
        network_send_object(obj, true)
        local msg = string.format('%s%s 拿到了 %s 旗帜!', team_color_str(s.team), strip_colors(np.name), team_name_str(s.team))
    ...
    obj.oPosZ = m.pos.z
    network_send_object(obj, true)
    local msg = string.format('%s%s 夺取了 %s%s%s 旗帜!', team_color_str(s.team), strip_colors(np.name), team_color_str(obj.oArenaFlagTeam), team_name_str(obj.oArenaFlagTeam), team_color_str(s.team))
    send_arena_flag(obj.oArenaFlagTeam, np.globalIndex, msg)
    return true
end
...
function bhv_arena_flag_check_drop(obj)
    ...
    if mario_health_float(m) <= 0 then
        ...
        obj.oTimer = 0
        network_send_object(obj, true)
        local msg = string.format('%s%s 掉落了 %s%s%s 旗帜!', team_color_str(s.team), strip_colors(np.name), team_color_str(obj.oArenaFlagTeam), team_name_str(obj.oArenaFlagTeam), team_color_str(s.team))
        send_arena_flag(obj.oArenaFlagTeam, ARENA_FLAG_INVALID_GLOBAL, msg)
        return
    end
end
