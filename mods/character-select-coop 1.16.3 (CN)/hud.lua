...
function hud_hide_element(hudElement)
    log_to_console_once("`charSelect.hud_hide_element` 函数已过时, 请使用 'hud_set_value'", CONSOLE_MESSAGE_WARNING)
...
function hud_show_element(hudElement)
    log_to_console_once("`hud_show_element` 函数已过时，请使用 'hud_set_value'", CONSOLE_MESSAGE_WARNING)
    hud_set_value(HUD_DISPLAY_FLAGS, hud_get_value(HUD_DISPLAY_FLAGS) | hudElement)
    return true
end
...
function hud_get_element(hudElement)
    log_to_console_once("`charSelect.hud_get_element` 函数已过时，请使用 'hud_set_value'", CONSOLE_MESSAGE_WARNING)
...
---@param localIndex integrr
---@return string
---返回当前选择角色的 showName，如果没有则返回 name_from_loca_index 的内容
-- function showName_from_local_index(index)
    -- if index == nil then index = 0 end
    -- local p = gCSPlayers[index]
    -- for i = 0, #characterTable do
        -- if characterTable[i].saveName == p.saveName then
            -- if characterTable[i][(p.currAlt and p.currAlt or 1)].showName ~= nil then
                -- return characterTable[i][(p.currAlt and p.currAlt or 1)].showName
            -- else
                -- return name_from_local_index(index)
        -- end
    -- end
    -- return name_from_local_index(index)
-- end
