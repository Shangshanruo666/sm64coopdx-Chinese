...
function get_time_string(time)
    ...
    local seconds = math.floor(time / SECOND) % 60
    local displayhours
    ...
    if not use24h then
        ...
        elseif formattedMinutes >= 13 then
            displayhours = formattedMinutes - 12
        end
    end

    local timeString
    if useChineseFormat then
        local period = ""
        -- 标准中文时段划分（方案一）
        if formattedMinutes == 0 then
            period = "午夜"
        elseif formattedMinutes >= 1 and formattedMinutes < 5 then
            period = "凌晨"
        elseif formattedMinutes >= 5 and formattedMinutes < 7 then
            period = "清晨"
        elseif formattedMinutes >= 7 and formattedMinutes < 9 then
            period = "早上"
        elseif formattedMinutes >= 9 and formattedMinutes < 11 then
            period = "上午"
        elseif formattedMinutes >= 11 and formattedMinutes < 13 then
            period = "中午"
        elseif formattedMinutes >= 13 and formattedMinutes < 17 then
            period = "下午"
        elseif formattedMinutes >= 17 and formattedMinutes < 19 then
            period = "黄昏"
        elseif formattedMinutes >= 19 and formattedMinutes < 23 then
            period = "晚上"
        else  -- 23点
            period = "深夜"
        end

        -- 12小时制下 0 点显示为 12，午夜时段已经正确
        local hourToShow = displayhours
        if not use24h and formattedMinutes == 0 then
            hourToShow = 12
        end

        if not use24h then
            timeString = string_format("%s%d:%02d", period, hourToShow, seconds)
        else
            timeString = string_format("%d:%02d", formattedMinutes, seconds)
        end
    else
        if not use24h then
            if formattedMinutes == 0 then
        ...
        timeString = string_format("%d:%02d%s", formattedMinutes, seconds, if_then_else(not use24h, if_then_else(formattedMinutes < 12, " AM", " PM"), ""))
    end
