-- name: Day Night Cycle DX (CN)
-- incompatible: light day-night-cycle
-- description: Day Night Cycle DX v2.5.1\nBy \\#ec7731\\Agent X\n\n\\#dcdcdc\\Special thanks to \\#344ee1\\eros71\\#dcdcdc\\ for salvaging\nthe mod files.\n\n\nChinese language and functions produced by \\#157bb2\\Shangshanruo
...
local function on_set_command(msg)
    if dayNightCycleApi.lockTime then
        ...
        djui_chat_message_create("\\#ffa0a0\\[昼夜交替] 昼夜交替设置已被另一个MOD锁定。")
        return
    end

    if msg == "" then
        djui_chat_message_create("/time \\#00ffff\\set\\#ffff00\\ [时间]\\#dcdcdc\\ 设置时间")
        return
    end
    ...
    elseif msg == "sunset" then
        ...
        if amount ~= nil then
            ...
            djui_chat_message_create("[昼夜交替] 将时间设置为 " .. math_floor(gGlobalSyncTable.time / SECOND))
        else
            djui_chat_message_create(string.format("\\#ffa0a0\\[昼夜交替] 无法将时间设置为 '%s'", msg))
        end
    end
...
local function on_add_command(msg)
    ...
    local amount = tonumber(msg)
    if amount == nil then
        djui_chat_message_create("/time \\#00ffff\\add\\#ffff00\\ [数值]\\#dcdcdc\\ 添加时间")
        return
    end
    local oldTime = gGlobalSyncTable.time
...
local function on_scale_command(msg)
    ...
    if scale == nil then
        djui_chat_message_create("/time \\#00ffff\\scale\\#ffff00\\ [速率]\\#dcdcdc\\ 设置时间流速")
        return
    end
    ...
    djui_chat_message_create("[昼夜交替] 将时间流速设置为 " .. scale)

    save_time()
end

local function on_query_command()
    djui_chat_message_create(string.format("[昼夜交替] 时间为 %d (%s) ,第 %d 天", math_floor(gGlobalSyncTable.time / SECOND), get_time_string(gGlobalSyncTable.time), get_day_count()))
end
...
local function on_sync_command()
    ...
    djui_chat_message_create("[昼夜交替] 尝试将游戏时间与现实生活时间同步...")
...
local function on_sync_sun_command()
    ...
    if dayNightCycleApi.lockSunHours then
        play_sound(SOUND_MENU_CAMERA_BUZZ, gGlobalSoundSource)
        djui_chat_message_create("\\#ffa0a0\\[昼夜交替] 太阳时间已被另一个MOD锁定。")
        return
    end
...
local function on_music_command()
    ...
    djui_chat_message_create("[昼夜交替] 夜间音乐状态: " .. on_or_off(playNightMusic))
...
local function on_display_time_command()
    ...
    djui_chat_message_create("[昼夜交替] 时间显示状态: " .. on_or_off(displayTime))
...
local function on_time_command(msg)
    ...
    if args[1] == "set" then
        if not network_is_server() then
            djui_chat_message_create("\\#ffa0a0\\[昼夜交替] 你没有权限执行 /time set")
        else
            on_set_command(args[2] or "")
        end
    elseif args[1] == "add" then
        if not network_is_server() then
            djui_chat_message_create("\\#ffa0a0\\[昼夜交替] 你没有权限执行 /time add")
        else
            on_add_command(args[2] or "")
        end
    elseif args[1] == "scale" then
        if not network_is_server() then
            djui_chat_message_create("\\#ffa0a0\\[昼夜交替] 你没有权限执行 /time scale")
        else
    ...
    elseif args[1] == "sync" then
        if not network_is_server() then
            djui_chat_message_create("\\#ffa0a0\\[昼夜交替] 你没有权限执行 /time sync")
        else
            on_sync_command()
        end
    ...
    elseif args[1] ~= nil then
        djui_chat_message_create("\\#ffa0a0\\[昼夜交替] 无法识别的命令 '" .. args[1] .. "'")
    else
        if not network_is_server() then
            djui_chat_message_create("\\#ffa0a0\\[昼夜交替] 你没有权限启用或禁用昼夜交替")
        else
            gGlobalSyncTable.dncEnabled = not gGlobalSyncTable.dncEnabled
            djui_chat_message_create("[昼夜交替] 状态: " .. on_or_off(gGlobalSyncTable.dncEnabled))
        end
    end
...
local function on_set_time_scale(index, value)
    ...
    update_mod_menu_element_name(index, "时间流速: " .. value)
end
...
local savedChineseFormat = mod_storage_load_bool("ChineseFormat")
if savedChineseFormat ~= nil then
    useChineseFormat = savedChineseFormat  -- 如果存在保存值，使用它
else
    useChineseFormat = false  -- 如果没有保存值，默认为 false
end

local function toggleTimeFormat()
    useChineseFormat = not useChineseFormat  -- 切换时间显示格式
    mod_storage_save_bool("ChineseFormat", useChineseFormat)  -- 保存设置
    
    -- 使用 DJUI 聊天信息展示当前时间显示格式状态
    local message = "[昼夜交替] 时间显示格式已切换为: " .. (useChineseFormat and "中文格式 (此功能汉化版特有)" or "英文格式 (此功能汉化版特有)")
    djui_chat_message_create(message)  -- 在聊天窗口显示消息
end
...
hook_chat_command("time", "\\#00ffff\\[set|add|scale|query|24h|sync|sync-sun|music|display-time]\\#dcdcdc\\ - 昼夜交替DX的命令句柄 \\#7f7f7f\\(保留为空可启用或禁用昼夜交替)", on_time_command)
...
hook_mod_menu_text(string.format("版本 %d.%d.%d", DNC_VERSION_MAJOR, DNC_VERSION_MINOR, DNC_VERSION_PATCH))

if network_is_server() then hook_mod_menu_checkbox("启用昼夜交替", gGlobalSyncTable.dncEnabled, on_set_dnc_enabled) end

modMenuDisplayTime = hook_mod_menu_checkbox("在 HUD 上显示时间", displayTime, on_display_time_command)
modMenu24h = hook_mod_menu_checkbox("24小时制", use24h, on_24h_command)
modMenuMusic = hook_mod_menu_checkbox("夜间音乐", playNightMusic, on_music_command)

if network_is_server() then
    modMenuSyncSun = hook_mod_menu_checkbox("使用现实生活中的日出/日落时间", syncSun, on_sync_sun_command)

    modMenuTimeScale = hook_mod_menu_slider("时间流速: " .. gGlobalSyncTable.timeScale, gGlobalSyncTable.timeScale, 0, 20, on_set_time_scale)
    modMenuTimeModifier = hook_mod_menu_inputbox("修改时间", "0", 8, on_set_time_modifier)
    hook_mod_menu_button("添加时间", on_add_hour)
    hook_mod_menu_button("与现实生活时间同步", on_sync_command)
end

hook_mod_menu_button("查询时间", on_query_command)

-- 添加时间显示格式切换按钮
hook_mod_menu_button("切换中英文时间显示格式", toggleTimeFormat)