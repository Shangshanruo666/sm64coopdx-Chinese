...
function render_game_mode()
    ...
    if gGlobalSyncTable.gameMode == GAME_MODE_DM then
        txt = string.format("%s | 谁率先达到 %d 击杀呢?", gGameModes[gGlobalSyncTable.gameMode].name, scoreCap)
    elseif gGlobalSyncTable.gameMode == GAME_MODE_TDM then
        txt = string.format("%s | 哪个队伍率先达到达到 %d 击杀呢?", gGameModes[gGlobalSyncTable.gameMode].name, scoreCap)
    elseif gGlobalSyncTable.gameMode == GAME_MODE_CTF then
        txt = string.format("%s | 哪个队伍将对方旗帜带回己方阵营的次数率先到达 %d 呢?", gGameModes[gGlobalSyncTable.gameMode].name, scoreCap)
    elseif gGlobalSyncTable.gameMode == GAME_MODE_FT then
        txt = string.format("%s | 谁的分数率先达到 %d 呢?", gGameModes[gGlobalSyncTable.gameMode].name, scoreCap)
    elseif gGlobalSyncTable.gameMode == GAME_MODE_TFT then
        txt = string.format("%s | 哪个队伍的分数率先达到 %d 呢?", gGameModes[gGlobalSyncTable.gameMode].name, scoreCap)
...
function render_local_rank()
    ...
    local rankTxt = ""
    if gGameModes[gGlobalSyncTable.gameMode].useScore then
        rankTxt = string.format("%s | %d 分数 | %d 击杀 | %d 死亡", rank_str(s.rank), s.score, s.kills, s.deaths)
    else
        rankTxt = string.format("%s | %d 击杀 | %d 死亡", rank_str(s.rank), s.kills, s.deaths)
    end
    local scale = 0.25
